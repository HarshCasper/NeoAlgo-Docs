...
// for finding the minimum in between
int min(int a, int b)
{
    if (a < b)
        return a;
...
// function for performing coin change
void coinChange(int n, int amount)
{
    int Max = amount + 1;
    // making array of number of coins
    // making a dp array
    int coins[n], dp[Max];
    int i, j;
...
- [Ansh Mishra](https://github.com/anshmishra010) for the [C implementation]
