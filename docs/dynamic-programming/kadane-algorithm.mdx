...
```
arr = -4 -1 2 3 5 6 1 -4 -6 
sum_till_now = arr[0] = -4        
max_sum = arr[0] = -4
...
int kadane(int arr[], int n) 
{
    ...
    while (i < n) 
    {
        // Equating sum_till_now to 0
        // if it holds a negative value

        if (sum_till_now < 0) sum_till_now = 0;
        // Adding current element to sum_till_now

        sum_till_now += arr[i];
        // Equating max_sum to sum_till_now
        // if sum_till_now holds a greater value
        // than max_sum
