...
We have to determine if the given array of positive integers can be divided into two subsets which need not be equal such that the sum of elements of both the subsets is equal. Each element must belong to exactly one subset only.
...
1. Make an array `dp[n+1][sum/2+1]` where row represents array values and column represents sum values from `0` to `sum` (C++ code follows this approach) or `dp[sum/2+1][n+1]` where column represents array values and row represents sum values from `0` to `sum` (Python code follows this approach). We will use first approach for explaining the algorithm.`dp[i][j]` represents that whether we can find the sum `j` from the subset of array elements upto index `i`. 
2. Initialize first column with `true` as we can always find sum `0` by taking no element from the array.
3. Initialize the first row with `false` except `dp[0][0]` as its not possible to have the sum `j` by not taking any element from the array.
4. Each element of the array has two choices, either to be included or to be excluded. `dp[i-1][j]` for `i>=1`, represents whether its possible to find the sum `j` without including that element. `dp[i-1][j-a[i-1]]` represents whether its possible to find the sum `j` by including that element.
5. For `1<=i<=n` and `1<=j<=n` calculate `dp[i][j]` using `dp[i][j]=dp[i-1][j]||dp[i-1][j-a[i-1]]`.
6. `dp[n][sum/2]` represents the final answer to the problem. If it is `true` then its possible to parition the given array ito two parts having equal sum, else, we can't partition.
