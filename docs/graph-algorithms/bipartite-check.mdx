...
- Create an empty array `color` which indicates the color of the vertex.
- Initialize the `color` array to `-1` which means at the start all vertices are uncolored.
- From every uncolored vertex 
    - Perform BFS. Assign the vertex the color `1` (putting into one set `U`). 
    - Color its neighbors with color `color ^ 1` (putting into the other set `V`).
    - If we try to color a neighbor which has already been colored before and both the vertices of the edge belong to the same set, either `U` or `V`, then we can conclude that the graph is NOT Bipartite. 
