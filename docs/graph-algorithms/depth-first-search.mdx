...
Depth First Search or DFS uses stack data structure. The algorithm starts with an initial node of the Graph and goes deeper and deeper until it finds a node which has no children called the **dead node**. Then it starts backtracking from the dead node towards the most recent
node that is yet to be completely unexplored. Here the unvisited nodes are named as **discovery edges** while the ones that leads to an already visited node are called **block edges**.
