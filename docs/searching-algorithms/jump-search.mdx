...
Jump search, also known as block search is a searching algorithm for sorted lists. It locates an item by jumping ahead a fixed number of elements instead of traversing through each element.
...
After finding a correct block it finds the item using a linear search algorithm.
