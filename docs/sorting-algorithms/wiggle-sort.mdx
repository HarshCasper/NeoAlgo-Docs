...
Given an unsorted array `A[n]`, Wiggle Sort algorithm sorts the array `A` such that `A[0] < A[1] > A[2] < A[3] > A[4] . . .`. Wiggle sort sorts the array in a waveform, i.e., the wiggle sorted array resembles a wave-like structure.
