...
Heap Sort is a sorting algorithm which extracts the maximum or minimum element from the array and swap it with the last element of the array 
for sorting in ascending or descending order N times.It is similar to Selection Sort, the only difference is that it uses Heap data structure 
for finding minimum or maximum element. It is a In-Place and Non-Stable Sorting Algorithm.
