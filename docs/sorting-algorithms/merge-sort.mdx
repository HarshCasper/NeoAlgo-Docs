...
To understand better, lets take an example of an unsorted array
...
We notice that ``12`` and ``33`` are already sorted, we directly combine the list. Next we move to ``28`` and ``10``. Since ``28`` is greater than  ``10`` we put ``10`` first, followed by ``27``. Similarly, we need to change the order of ``35`` and ``19`` where as ``42`` and ``44`` are combined without any changes.
...
In the next iteration, we compare the 2 lists together, and merge them into a list placing all in a sorted order.
