...
Rail fence cipher is also known as the zig-zag cipher. The name clearly suggests we compute in a zig-zag manner. We take the given plaintext and the key and store them in a `2D` array stepwise. The depth of steps is dependent on the key. Then we have the encrypted text by concatenating the characters of each row.
...
- We take the plaintext and the key from the user.
- we create an array where we compute and store the plaintext in a zig-zag form corresponding to the given key.
- Then we concatenate each character row-wise.
- we arrive at our encrypted text.  
