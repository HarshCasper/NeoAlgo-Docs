...
```
  ...
  - Also, the encryption process can be done by using the algebraic formula. Convert `A` to `Z` to numbers. `A` is converted to `0`, `B` to `1`, . . . . . , `Z` to `25`. ciphertext = (plaintext + keyword) % 26. Here, `N` becomes `13`. `k` becomes `10`. So, the ciphertext becomes, (13 + 10) % 26 = 23. Converting it back to ASCII, we get ciphertext as `X``.
