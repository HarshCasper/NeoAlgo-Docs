...
If the number we calculate by the sum of squares appears again in the loop. In such a case, we don't see the loop terminating. So such a number is not a happy number.
