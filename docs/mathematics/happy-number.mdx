...
A number is called a happy number if it converges to 1 after a sequential operation wherein each step we replace the original number by the sum of the squares of its individual digits.When we have a digit given to us, we perform the operation, the sum of squares of its individual digits but in some cases, we might arrive at the same number which we already came across. In such cases, we can clearly see that it would never terminate so such numbers are not considered as happy numbers.

## Explanation

Given a number, we take the sum of squares of its individual digits and replace it with the number. Now if we arrive at the same number after a sertain number iterations. we say such numbers are not considered as happy numbers.
