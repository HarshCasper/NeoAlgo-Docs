...
The transpose of a matrix is an operator which flips a matrix over its diagonal, It is obtained by switching the row and column indices of the matrix. It is denoted by Aᵀ. Let's have an example:
...
Here `n` and `m` are the number of rows and columns.
