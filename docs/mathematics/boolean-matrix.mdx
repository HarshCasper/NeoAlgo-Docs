...
Boolean Matrix refers to the matrix which only contains `0` and `1`. We are given a boolean matrix of size `N X N`, we need to modify the matrix such that if a matrix cell `matrix[i][j]` is `1`, then make all cells of i<sup>th</sup> row and j<sup>th</th> column as `1`.
