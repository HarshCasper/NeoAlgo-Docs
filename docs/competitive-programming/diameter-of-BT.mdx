...
The maximum no. of edges between the two farthest nodes in a tree is called the diameter of a binary tree.
...
As you can see in the tree of case 1, the longest path is from Node `G -> F`, including root node `A`. So, the diameter for this tree will be `5` as there are five edges between Node `G` and `F`. Alternatively, we can count the number of nodes in the path between Node `G` & `F` and then subtracting `1` from the total nodes to get the number of edges which in this case is the diameter of the tree.
...
* Recursively find the height of left sub-tree and right sub-tree.
* Recursively find the diameter of left sub-tree and right sub-tree.
* Check whether the diameter passes through the root node
  ...
  * Else, diameter will be `max(ld,rd)` where ld and rd are the diameters of left sub-tree and right sub-tree respectively.
* Return the maximum value of above steps.
