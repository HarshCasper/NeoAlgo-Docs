---
title: Factorial of very large number 
...
largeFactorial(n):
	Set rsize to 1
  	FOR x=2 to n
		Set rsize to multiply(x, res, rsize)
	ENDFOR
...
multiply(n,res,rsize):
	Set carry to 0
  	FOR i=0 to rsize-1
		Set product to res[i] * x + carry
		Set res[i] to product % 10
		Set carry to product/10
	ENDFOR
	WHILE carry exists
		push carry%10 to res
		divide carry by 10
		increment rsize
	ENDWHILE
	RETURN rsize
