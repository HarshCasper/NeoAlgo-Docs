...
void Search(Node *p,int key)  //Searching operation takes key as input and search for key in BST
{
    if(p==NULL)
    {
...
void InOrder(Node *p)   //Inorder traversal is used because inorder of BST gives Sorted output
{
    if(p)
    {
        InOrder(p->lchild);
